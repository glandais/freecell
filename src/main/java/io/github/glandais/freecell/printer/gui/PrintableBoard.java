...
import static io.github.glandais.freecell.printer.gui.Constants.*;
...
public class PrintableBoard extends ArrayList<PrintableCard> {
    ...
    private void addCards(Board board) {
        for (SuitePilesEnum suitePilesEnum : SuitePilesEnum.values()) {
            Pile pile = board.getPile(suitePilesEnum.getPilesEnum());
            int i = 0;
            for (CardEnum cardEnum : pile.getVisible()) {
                add(new PrintableCard(cardEnum, getFoundationPosition(suitePilesEnum.ordinal(), i++), true, -i));
            }
        }
